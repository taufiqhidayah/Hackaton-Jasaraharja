...
include_once "koneksinews.php";
	...
	$all = mysqli_query($con, "SELECT * FROM gbr WHERE tipe='berita' ORDER BY id DESC");
	$count_all = mysqli_num_rows($all);
	$query = mysqli_query($con, "SELECT * FROM gbr WHERE tipe='kecelakaan' AND kondisi='jemput' ORDER BY id DESC LIMIT $offset,10");
	...
	if($count<10){
		if($count==0){
			...
			$query = mysqli_query($con, "SELECT * FROM gbr WHERE tipe='kecelakaan' AND kondisi='jemput' ORDER BY id DESC LIMIT $offset,$count");
			$count = mysqli_num_rows($query);
			if(empty($count)){
				$query = mysqli_query($con, "SELECT * FROM gbr WHERE tipe='kecelakaan' AND kondisi='jemput' ORDER BY id DESC LIMIT 0,10");
				$num = 0;
			}else{
				$num = $offset;
			}
